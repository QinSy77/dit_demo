...
from diffusion.model.clip_text_emb import FrozenCLIPEmbedder
...
def extract_caption_t5_job(item):
    ...
    with torch.no_grad():
        ...
        try:
            ...
            caption_emb = clip.encode(caption)
...
def extract_caption_t5():
    ...
    ###基本只把t5编码改成clip的其他没变
    clip = FrozenCLIPEmbedder(path="/mnt/workspace/qinshiyang/.cache/mymodel/stable-diffusion-xl-base-1.0", device="cuda", max_length=77)
...
def extract_img_vae():
    vae = AutoencoderKL.from_pretrained(args.vae_models_dir).to(device)
...
def extract_img_vae_multiscale(bs=1):
    ...
    vae = AutoencoderKL.from_pretrained(args.vae_models_dir).to(device) #madebyollin/sdxl-vae-fp16-fix #stabilityai/sd-vae-ft-ema
...
def get_args():
    ...
    parser.add_argument('--vae_models_dir', default='stabilityai/sd-vae-ft-ema', type=str)

    # madebyollin/sdxl-vae-fp16-fix #stabilityai/sd-vae-ft-ema #stabilityai/sdxl-vae
    
    parser.add_argument('--json_path', type=str,default="/mnt/workspace/qinshiyang/PixArt-alpha/data/210data_sd15_clip/partition/210data_info.json") #t5和单进程需要用

    parser.add_argument('--t5_save_root', default='/mnt/workspace/qinshiyang/PixArt-alpha/data/210data_sd15_clip/caption_feature_wmask', type=str)
    parser.add_argument('--vae_save_root', default='/mnt/workspace/qinshiyang/PixArt-alpha/data/210data_sd15_clip/img_vae_features', type=str)
    parser.add_argument('--dataset_root', default='', type=str) #dataset_root+图像地址就是dataset读取的方式 所以需要置为空
    #dataset_root+图像地址就是dataset读取的方式

    ### for multi-scale(ms) vae feauture extraction
    parser.add_argument('--json_file', type=str,default="210data_info.json")
